                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-xl-6">
                        <div class="bg-light rounded h-100 p-4 auto-width">
                            <h3 class="mb-4">ΥΛΙΚΑ ΠΡΟΣ ΠΡΟΩΘΗΣΗ</h3>
                            ...
                            <div id="search">
                                ...
                                <form method="post" action="noview_akirosiaitimatos.php" class="form-inline"> <!--ΓΙΑ ΑΚΥΡΩΣΗ ΑΙΤΗΜΑΤΟΣ-->
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        try{
                                            if($_SERVER['REQUEST_METHOD'] == 'GET'){
                                                ...
                                                $stmt = mysqli_prepare($dbcon, 'SELECT * from (not_deleted_product_info p inner join ekkremeis_xreoseis ex on p.barcode=ex.barcode) inner join monada m on ex.id_monada_after=m.id_monada where id_monada_before=?');
                                                ...
                                                $query = "SELECT * FROM (not_deleted_product_info p inner join ekkremeis_xreoseis ex on p.barcode=ex.barcode) inner join monada m on ex.id_monada_after=m.id_monada 
                                                where id_monada_before=? AND (p.barcode IN (SELECT pr.barcode FROM products pr WHERE id_tmima IN (SELECT id_tmima FROM tmima WHERE id_monada=?)
                                                ...
                                                mysqli_stmt_bind_param($stmt, 'iis', $monadaid, $monadaid, $typos);  
                                    ...
                                    <button type="submit" class="btn btn-danger" <?php if ($aa==1){echo 'disabled';} ?>><i class="fa fa-ban me-2"></i>Ακύρωση Προώθησης</button>
                                    </form>
